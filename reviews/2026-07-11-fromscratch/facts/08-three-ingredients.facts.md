...
- No survey has everything. The CPS captures wages well and capital income poorly (people underreport dividends and gains, and the very wealthy are undersampled); it top-codes very high earnings [@larrimore2008topcoding], compressing the top tail exactly where high-earner policies bite; and it omits tax-relevant detail like itemized deductions and retirement contributions entirely. (Marker resolved — Larrimore et al. 2008.)
...
- Milestone the approach is measured against: the Enhanced CPS that PolicyEngine released in August 2025, which fused five datasets — the CPS (income and employment backbone), the Survey of Consumer Finances (wealth), the Consumer Expenditure Survey (spending), the Residential Energy Consumption Survey (energy use), and the American Time Use Survey (hours) — then calibrated the result to 9,168 administrative targets, cutting the average deviation from those totals from 8.3 percent to 0.2 percent.
  - **[VERIFY: publish or soften]** — this is the fullest statement of the Enhanced CPS figures (five named datasets, 9,168 targets, 8.3%→0.2%), and they are author-internal August-2025 populace-era numbers that are NOT publicly verifiable. The previously cited `[@policyengine2022enhanced]` is a Dec 2022 blog post that predates and does not contain them (the flagged key-year mismatch — confirmed real), so the citation was removed. The five datasets named here are plausible but the researcher could not confirm them in one published place. Point at the actual August-2025 release notes / policyengine-us-data changelog, or soften to the publicly documented figures. See research/part2-verification.md correction C6.
...
- **Marker status after the 2026-07-11 verification merge:** RESOLVED: CPS top-coding → [@larrimore2008topcoding]. CHANGED: the `policyengine2022enhanced` key-year mismatch → citation removed; the Enhanced CPS figures flagged **[VERIFY: publish or soften]** as author-internal. STILL OPEN (keep): number of states with EITC variants [NEEDS CITATION]; PolicyEngine US parameter count [NEEDS CITATION]; EUROMOD UK component file size [NEEDS CITATION]; Axiom coverage counts [VERIFY counts at publication].
