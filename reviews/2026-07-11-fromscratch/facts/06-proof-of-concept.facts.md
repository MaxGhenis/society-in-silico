...
- Tax-Calculator shipped a pandemic release, version 3.2.1, in August 2021 [@taxcalculator2021v321]. (Marker resolved — GitHub release "3.2.1 (2021-08-06)".)
...
- Simple case: an outside contributor comparing results against IRS tables caught a transposed phase-out rate — 21.60 percent entered where the EITC's two-child schedule specifies 21.06 percent [@tpc2024eitcparameters] [VERIFY: real incident]. (The 21.06% two-child phase-out rate is now verified via IRS Rev. Proc. 2024-40 / TPC; the [VERIFY: real incident] marker stays because whether this specific transposition happened is not on the record. A documented alternative "whose-bug-is-it?" incident exists — a UKMOD savings-allowance bug filed publicly [@ukmod2026bugfiling] — see research/part2-verification.md.)
...
- For the UK model, HM Treasury eventually ran its own evaluation and found that "almost 60 percent" of National Insurance calculations fell within 0.5 percent of the government's own model [@hmt2025policyengine]. (Wording corrected per primary source — the ATRS says "NI calculations proved to be very accurate, with almost 60% of datapoints falling within 0.5% of one another"; see research/part2-verification.md C3.)
...
- In 2025 the National Science Foundation awarded PolicyEngine a Pathways to Enable Open-Source Ecosystems (POSE) Phase I grant — award #2518372 to the PSL Foundation, $299,974 (rounded to "$300,000"), award date 15 August 2025 [@nsf2025pose; @policyengine2025nsfpose]. (Marker resolved and year corrected 2024→2025 per primary source; this must be decoupled from the April 2024 fifty-state launch, which is a separate, earlier event — see research/part2-verification.md correction C1.)
- In April 2024, PolicyEngine launched income tax modeling for all fifty states and DC, the work of more than a hundred open-source contributors. (Contributor-count marker resolved: confirmed via the GitHub API — policyengine-us alone has ~141 contributors as of the 2026-07-11 verification; the "April 2024" month is confirmed only at year granularity by the 2024 year-in-review [@policyengine2024usreview], so treat the month as [VERIFY] or soften to "in 2024.")
...
- In August 2025 PolicyEngine launched the Enhanced Current Population Survey, integrating five datasets and calibrating to 9,168 administrative totals; the pipeline paired Quantile Regression Forests for imputation with gradient-descent reweighting and cut deviations from official statistics by 97 percent. **[VERIFY: publish or soften]** — these figures (9,168 targets, 97% deviation cut) are author-internal, August-2025 populace-era numbers that are NOT publicly verifiable; the previously cited `[@policyengine2022enhanced]` is a December 2022 blog post that predates and does not contain them, so its citation was removed. Either point at the actual August-2025 release notes / policyengine-us-data changelog that documents 9,168 targets, or soften to the publicly documented figures (the beta post = 3 sources / ~90 targets; the docs = 2 datasets / 2,813 targets). See research/part2-verification.md correction C6.
- That data line was later rebuilt as populace, described as the open commons of calibrated microdata the book returns to.
- In September 2025 PolicyEngine signed a memorandum of understanding with the National Bureau of Economic Research to build an open-source emulator of TAXSIM, the tax calculator that had powered academic research since the 1970s; more than 1,200 papers had relied on it [@policyengine2025nbertaxsim]. (Marker resolved — the NBER MOU post states "Over 1,200 papers have applied TAXSIM." Note an internal PolicyEngine drift: the NSF POSE post says "over 1,100.")
...
- In December 2024, HM Treasury published an Algorithmic Transparency Record (dated "Published 17 December 2024") disclosing that it had piloted PolicyEngine UK as a possible supplement to IGOTM, the closed model that had produced government policy estimates for decades [@hmt2025policyengine]. (Date corrected from "March 2025" per primary source — see research/part2-verification.md correction C2.)
...
- The Times reported that the tool could more accurately predict the outcomes of ministers' decisions and was expected to be used mainly in budget periods [@hmt2025policyengine] [VERIFY: Times citation mismatch — this paraphrase is attributed to the gov.uk ATRS (@hmt2025policyengine), not The Times; obtain the Times article's date/headline/author and add a distinct key, or re-attribute to the ATRS. Marker unresolved — see research/part2-verification.md]. (Reported/paraphrased in the source, not a verbatim Times quotation.)
- Nikhil Woodruff, PolicyEngine's co-founder, served as an "Innovation Fellow with 10DS" (the No 10 Downing Street data-science team) for roughly six months from summer 2025, building a `10ds-microsim` package on PolicyEngine that was "intensively validated against external projections and official forecasts" [@policyengine2026downingstreet] [VERIFY with Nikhil: role wording]. (Disclosure fact enriched with the public trace per research/part2-verification.md — it was a fellowship on the data-science team, clearly experimental, not a permanent government post; the [VERIFY with Nikhil] marker STAYS. Per the fact sheet, use with a disclosure at first mention.)
...
- **Marker status after the 2026-07-11 verification merge:**
  - RESOLVED: Tax-Calculator v3.2.1 date → [@taxcalculator2021v321]; NSF POSE award → [@nsf2025pose; @policyengine2025nsfpose] (and year corrected 2024→2025, decoupled from the 50-state launch); >100 contributor count → confirmed via GitHub API (~141 on policyengine-us); >1,200 TAXSIM papers → [@policyengine2025nbertaxsim]; the HM Treasury ATRS date → corrected to December 2024 and NI wording to "almost 60% … within 0.5% of one another."
  - CHANGED: the Enhanced CPS `policyengine2022enhanced` key-year mismatch → citation removed; figures flagged **[VERIFY: publish or soften]** as author-internal (see the Enhanced CPS fact above).
  - STILL OPEN (keep): the EITC transposition [VERIFY: real incident] (21.06% rate now sourced, incident not); Nikhil's role wording [VERIFY with Nikhil] (enriched with the "Innovation Fellow with 10DS" public trace but marker stays); the Atlanta Fed MOU wording [VERIFY at copyedit]; the Times-citation mismatch (new [VERIFY]); the "April" 2024 fifty-state month [VERIFY].
