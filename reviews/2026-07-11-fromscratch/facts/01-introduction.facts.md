...
- "I don't predict the future. I create it." is a line from Engerraund Serac, the antagonist of Westworld's third season. (uncited — TV source)
...
- "I don't predict the future. I create it." — Engerraund Serac (Westworld, season 3). (No citation key in the manuscript.)
...
- Required framing device #1 — the Rehoboam/Westworld frame: Serac's closed system that "predicts the future by creating it" is the book's antagonist metaphor; it opens the introduction and Ch 17 returns to it ("Serac's closed system versus the democratic alternative"). Preserve the frame and the closed-vs-open contrast.
...
- Westworld quotations carry no citation keys in the manuscript; if a rewrite keeps them, they may need attribution added (currently in-text attribution to Serac / "the show" only).
